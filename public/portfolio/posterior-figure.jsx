...
function PosteriorFigure({ variant = "posterior" }) {
  ...
  const densities = [
    ...
    {
      mu: 2.6 + driftMu * 0.4,
      sigma: 0.45 * driftSigma,
      color: "var(--slate)",
      band: "var(--slate-soft)",
      band2: "var(--slate-soft)",
      label: "Channel C",
      faded: true,
    },
  ];
  ...
  let yMax = 0;
  densities.forEach(d => {
    yMax = Math.max(yMax, pdf(d.mu, d.mu, d.sigma));
  });
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Posterior distribution of channel coefficients"
    >
      {/* zero line */}
      <line
        x1={xScale(0)}
      ...
      {/* main: 94% HDI band */}
      <path
        d={buildArea(main.mu, main.sigma, hdi94[0], hdi94[1])}
        fill="var(--olive-200)"
      />
      {/* 50% HDI band on top */}
      <path
        d={buildArea(main.mu, main.sigma, hdi50[0], hdi50[1])}
        fill="var(--olive-300)"
      />
      {/* main outline */}
      <path
        d={buildPath(main.mu, main.sigma)}
        fill="none"
      ...
      <line
        x1={xScale(main.mu)}
        x2={xScale(main.mu)}
        y1={yScale(pdf(main.mu, main.mu, main.sigma))}
        y2={PAD.t + innerH}
      ...
      {/* mean callout */}
      <g
        transform={`translate(${xScale(main.mu) + 8}, ${yScale(pdf(main.mu, main.mu, main.sigma)) - 4})`}
      >
        <text
          fontFamily="var(--font-mono)"
          fontSize="10"
...
function ForestPlot() {
  ...
  const channels = [
    ...
    { name: "podcast", mean: -0.04, lo: -0.18, hi: 0.1 },
  ];
  const xMin = -0.3,
    xMax = 0.85;
  const xScale = x => PAD.l + ((x - xMin) / (xMax - xMin)) * innerW;
  ...
  return (
    <svg
      ...
      preserveAspectRatio="xMidYMid meet"
      role="img"
      aria-label="Forest plot of channel coefficients"
    >
      <text
        x={PAD.l}
        y={20}
        ...
        return (
          <g key={c.name}>
            ...
            <line
              ...
              y1={y}
              y2={y}
              stroke={isNeg ? "var(--rust)" : "var(--olive-600)"}
              strokeWidth="2"
            />
            <line
              x1={xScale(lo)}
            ...
            <circle
              ...
              cy={y}
              r="4"
              fill={isNeg ? "var(--rust)" : "var(--olive-700)"}
              stroke="var(--canvas)"
              strokeWidth="1.5"
            />
          </g>
...
function TracePlot() {
  ...
  const seriesPath = (seed, target) => {
    ...
    for (let i = 0; i <= N; i++) {
      ...
      const noise =
        Math.sin(i * 0.31 + seed) * 0.18 +
        Math.sin(i * 0.097 + seed * 1.7 + phase) * 0.1 +
        Math.cos(i * 0.21 + seed * 0.6) * 0.07;
  ...
  return (
    <svg
      ...
      preserveAspectRatio="xMidYMid meet"
      role="img"
      aria-label="Trace plot of MCMC chains"
    >
      <text
        x={PAD.l}
        y={18}
        ...
        {chains.map((c, idx) => {
          ...
          return (
            <g key={idx}>
              ...
              />
              <path
                d={seriesPath(idx * 1.7 + 0.4, 0.3 + idx * 0.05)}
                fill="none"
                stroke={c.color}
                strokeWidth="1.1"
                opacity="0.85"
